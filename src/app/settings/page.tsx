      ...
      <div className="glass-panel rounded-2xl p-6 lg:p-7 space-y-6 border-indigo-200/80 bg-gradient-to-br from-indigo-50/40 via-white to-white">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold border transition-all cursor-pointer ${autonomousMode
        ...
        <div className="flex flex-col space-y-5 text-xs">
          {/* 1. Daily Application Cap */}
          <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/80 space-y-3">
            <label className="block font-mono text-slate-700 font-medium flex items-center gap-1.5 text-xs">
              <ShieldCheck className="h-4 w-4 text-indigo-600" />
              <span className="font-semibold text-slate-800">Daily Application Cap (Anti-Spam Safeguard)</span>
            </label>
            <div className="flex items-center gap-3">
              <input
                type="range"
              ...
              />
              <span className="font-mono text-indigo-700 font-bold w-24 text-center bg-white border border-indigo-200 px-2.5 py-1 rounded-lg shadow-2xs">
                {dailyLimit} / day
              </span>
            </div>
            <p className="text-[11px] text-slate-500 leading-relaxed">
              Limits daily outgoing applications to keep your Gmail SMTP sender reputation completely safe from spam flags. Automatically resets every 24h at midnight.
            </p>
            <div className="pt-1">
              <button
                type="button"
                ...
                className="inline-flex items-center gap-2 px-3.5 py-1.5 text-xs font-semibold rounded-lg border border-indigo-200 bg-white text-indigo-700 hover:bg-indigo-50 transition-colors cursor-pointer disabled:opacity-50 shadow-2xs"
              >
                <RefreshCw className={`h-3.5 w-3.5 ${resettingDailyLimit ? 'animate-spin text-indigo-600' : 'text-indigo-600'}`} />
                <span>{resettingDailyLimit ? 'Resetting Count...' : "Reset Today's Application Count to 0"}</span>
              </button>
            </div>
          </div>

          {/* 2. Queue Empty Cooldown Interval */}
          <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/80 space-y-3">
            <label className="block font-mono text-slate-700 font-medium flex items-center gap-1.5 text-xs">
              <Clock className="h-4 w-4 text-indigo-600" />
              <span className="font-semibold text-slate-800">Queue Empty Cooldown Interval</span>
            </label>
            <div className="flex items-center gap-3">
              <input
                type="range"
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed">
              Minutes to wait before triggering the next automated discovery pass after all queued jobs have been processed.
            </p>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-indigo-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-bold border transition-all cursor-pointer self-start sm:self-auto ${emailOnly
      ...
      <div className="glass-panel rounded-2xl p-6 lg:p-7 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold border transition-all ${agentEnabled
